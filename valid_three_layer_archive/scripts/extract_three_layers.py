...
三层维度关系可以理解为：
- 外层（蓝色胜任表征维度）= 与海洋直接相关的可观察实践、项目、成果与活动（做了什么、在哪个海洋领域做）
- 中层（协同创新支撑维度）= 支撑这些实践的可迁移能力机制、方法经验、知识结构与资源组织方式（凭什么能做、怎样做成）
- 内层（深蓝精神内核维度）= 与海洋相关的内在认知、价值、使命与战略取向（为什么长期投入、相信什么、要推动什么）
...
{{
  "file": "{file_name}",
  "outer_layer": {{
    "summary": "1-2句话概括该人物在蓝色胜任表征维度方面的特征",
    "signals": [
  ...
  "middle_layer": {{
    "summary": "1-2句话概括该人物在协同创新支撑维度方面的特征",
    "signals": [
  ...
  "inner_layer": {{
    "summary": "1-2句话概括该人物在深蓝精神内核维度方面的特征",
    "signals": [
...
3. 严格区分三个维度：外层（蓝色胜任表征）= 与海洋直接相关的可观察实践、项目、成果与活动（做了什么）；中层（协同创新支撑）= 支撑海洋实践的能力机制、方法经验、知识结构与资源组织方式（凭什么能做、怎样做成）；内层（深蓝精神内核）= 与海洋相关的内在认知、价值、使命与战略取向（驱动什么、相信什么、要推动什么）。
4. 不要让预设的分类框架限制你的提取——文本中出现什么就提取什么，框架只是方向指引，不是边界。
5. 三个维度都必须紧扣海洋人才主题。禁止提取与海洋无关的一般管理能力、泛泛人格评价或普通履历标签。
